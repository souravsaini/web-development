  ...
  cartLogic() {
    ...
    cartContent.addEventListener("click", event => {
      if (event.target.classList.contains("remove-item")) {
        ...
        let subCount = event.target;
        let id = subCount.dataset.id;
        let tempItem = cart.find(item => item.id === id);
        tempItem.count -=1;
        if (tempItem.count > 0) {
          Storage.saveCart(cart);
          this.setCartValues(cart);
          subCount.previousElementSibling.innerText = tempItem.count;
        } else {
          cartContent.removeChild(subCount.parentElement.parentElement);
          this.removeItem(id);
        }
      }
    })
